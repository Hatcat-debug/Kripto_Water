Shader "Hidden/KriptoFX/KWS/UnderwaterModified"
{
	Properties
	{
			...
			half4 fragUnderwater(vertexOutput i) : SV_Target
{
    ...
    bool surfaceMask = (abs(waterMask - 0.75) < 0.1) && (volumeDepth.y > sceneZ);
    ...
    float3 worldPos = GetWorldSpacePositionFromDepth(uv, sceneZ);
    ...
    return float4(refraction, alpha);
}
